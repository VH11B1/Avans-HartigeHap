...
// This is the ols search Filter method used in navigator.
	searchGrid : function (p) {
		...
		return this.each(function() {
            ...
            function applyDefaultFilters(gridDOMobj, filterSettings) {
                ...
                contain default fetch settings. We will parse these and will populate the search with defaults.
                filterSettings - same settings object you (would) pass to $().jqGrid('searchGrid', filterSettings);
                */

                // Pulling default fetch settings out of postData property of grid's properties.:
                ...
                if (defaultFilters) {
                    ...
                    if (defaultFilters.rules) {
                        ...
                        for (; i < li; i++) {
                            ...
                            // we are not trying to counter all issues with fetch declaration here. Just the basics to avoid lookup exceptions.
	...
	filterGrid : function(gridid,p){
		...
		return this.each(function(){
			var self = this;
			this.p = p;
			if(this.p.filterModel.length === 0 && this.p.gridModel===false) { alert("No fetch is set"); return;}
